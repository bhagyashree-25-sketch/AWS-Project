...
2. **Create the Application Load Balancer**: Select "Application Load Balancer" and make it internet-facing. Configure a target group on port 80.
3. **Create the Auto Scaling Group**: Attach the Launch Template created in Step 1. Select the same VPC and public subnets. Attach the ALB created in Step 2 to the ASG.
Set the Group Size: Minimum (e.g., 2), Maximum (e.g., 6), and Desired (e.g., 2).
4. **Configure Scaling Policies**: Go to the ASG "Automatic Scaling" tab.Create a Target Tracking Scaling Policy. Select ASG Average CPU Utilization and set the target value (e.g., 50%).
